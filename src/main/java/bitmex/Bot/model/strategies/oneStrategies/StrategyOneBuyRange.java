...
public class StrategyOneBuyRange {
    private static StrategyOneBuyRange strategyOneBuyRange;
    ...
    public static StrategyOneBuyRange getInstance() {
        if (strategyOneBuyRange == null) strategyOneBuyRange = new StrategyOneBuyRange();
        return strategyOneBuyRange;
    }
    ...
    private synchronized void makeADecision() {
        ...
        if (inTheRangePrice() && inTheRangeTime()) {
            if (Gasket.getStrategyWorkOne() == 1) {
                ...
                if (Gasket.isStrategyOneBuyRangeFLAG()) {
                    Gasket.setStrategyOneBuyRangeFLAG(false);
                    new StrategyOneBuyThread(
                            ((int) (Math.round(Math.abs(Math.random() * 200 - 100)) * 39)) + "-SOBR", volume, getMin());
                }
            }
    ...
    private boolean inTheRangePrice() {
        double lowLevel = volume.getPrice() < bid.getPrice()
                ? volume.getPrice() - Gasket.getRangeLevel() : bid.getPrice() - Gasket.getRangeLevel();
