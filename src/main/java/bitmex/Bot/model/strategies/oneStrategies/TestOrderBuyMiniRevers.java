    ...
    public void run() {
        ...
        while (true) {
            ...
            if (priceAsk >= priseStopOrder) {
                ...
                ConsoleHelper.writeMessage(ID + " --- ИТОГО на счету СЕЛЛ MiniR --- " + Gasket.getPROFIT_Sell_MR());
                break;
            }
            ...
            if (priceBid <= priseTakeOrder) {
                ...
                ConsoleHelper.writeMessage(ID + " --- Сработал ТЕЙК ПРОФИТ MiniR ---- "
                        + DatesTimes.getDate());
                Gasket.setPROFIT_Sell_MR(Gasket.getPROFIT_Sell_MR() + Gasket.getTake());
                ConsoleHelper.writeMessage(ID + " --- ИТОГО на счету CEЛЛ MiniR --- " + Gasket.getPROFIT_Sell_MR());
                break;
            }
